# Changelog

## [2.8.0](https://github.com/ppat/homelab-ops-actions/compare/v2.7.0...v2.8.0) (2026-09-24)


### 🧹 Miscellaneous

* **dev-tools:** update jdx/mise (v2026.9.10 -&gt; v2026.9.12) ([#206](https://github.com/ppat/homelab-ops-actions/issues/206)) ([40f421b](https://github.com/ppat/homelab-ops-actions/commit/40f421bb86459e892ee226331a60691f70e8b288))


### ✨ Features

* **github-actions:** update trusted github-actions ([#202](https://github.com/ppat/homelab-ops-actions/issues/202)) ([0fc64c2](https://github.com/ppat/homelab-ops-actions/commit/0fc64c27177b912ed8513012ac5aafc033bbc966))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.12.15 -&gt; 0.12.17) ([#203](https://github.com/ppat/homelab-ops-actions/issues/203)) ([90af408](https://github.com/ppat/homelab-ops-actions/commit/90af408c3b38b72f9865a5471d80b412f0a20363))
* **dev-tools:** update astral-sh/uv (0.12.17 -&gt; 0.12.18) ([#207](https://github.com/ppat/homelab-ops-actions/issues/207)) ([e3dae00](https://github.com/ppat/homelab-ops-actions/commit/e3dae00668a525b21b83d9bc2c3a7eb187691fc1))

## [2.7.0](https://github.com/ppat/homelab-ops-actions/compare/v2.6.0...v2.7.0) (2026-09-16)


### 🧹 Miscellaneous

* **dev-tools:** update jdx/mise (v2026.9.1 -&gt; v2026.9.6) ([#196](https://github.com/ppat/homelab-ops-actions/issues/196)) ([e6a5411](https://github.com/ppat/homelab-ops-actions/commit/e6a541135c2b959d8b9218e506081bda8d222bec))
* **dev-tools:** update jdx/mise (v2026.9.6 -&gt; v2026.9.10) ([#200](https://github.com/ppat/homelab-ops-actions/issues/200)) ([955b3ef](https://github.com/ppat/homelab-ops-actions/commit/955b3ef11d7b64f37bb71eb1a2d8e5c6018b10da))


### ✨ Features

* **github-actions:** update jdx/mise-action (v4.2.5 -&gt; v4.3.0) ([#201](https://github.com/ppat/homelab-ops-actions/issues/201)) ([4fd3d8d](https://github.com/ppat/homelab-ops-actions/commit/4fd3d8d9e1564691bd3f7ff2d17cfbb01940438b))
* **github-actions:** update ppat/github-workflows (v8.0.0 -&gt; v8.1.0) ([#197](https://github.com/ppat/homelab-ops-actions/issues/197)) ([1840530](https://github.com/ppat/homelab-ops-actions/commit/18405306294dd74995443c80eaae0e94fa123aa9))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.12.13 -&gt; 0.12.15) ([#199](https://github.com/ppat/homelab-ops-actions/issues/199)) ([d097624](https://github.com/ppat/homelab-ops-actions/commit/d09762443a6277ee1e1b9544e5bc3dd4930eaff0))

## [2.6.0](https://github.com/ppat/homelab-ops-actions/compare/v2.5.0...v2.6.0) (2026-09-14)


### 🧹 Miscellaneous

* **dev-tools:** update jdx/mise (v2026.8.15 -&gt; v2026.9.1) ([#191](https://github.com/ppat/homelab-ops-actions/issues/191)) ([5ff4dc5](https://github.com/ppat/homelab-ops-actions/commit/5ff4dc534a23a8af374d8c432a9dbd154b418a43))


### ✨ Features

* add build-docker-image composite action ([#195](https://github.com/ppat/homelab-ops-actions/issues/195)) ([01ccc2f](https://github.com/ppat/homelab-ops-actions/commit/01ccc2ff5330388c232abd68a61d4f7807c6309a))
* **github-actions:** update ppat/github-workflows (v7.0.0 -&gt; v8.0.0) ([#188](https://github.com/ppat/homelab-ops-actions/issues/188)) ([33a975f](https://github.com/ppat/homelab-ops-actions/commit/33a975f92164512a020cf92ec4c3bf5eb84747df))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.12.10 -&gt; 0.12.11) ([#192](https://github.com/ppat/homelab-ops-actions/issues/192)) ([c88f4a8](https://github.com/ppat/homelab-ops-actions/commit/c88f4a8018c853baae6007aaca9b4a15a7ae2f1b))
* **dev-tools:** update astral-sh/uv (0.12.11 -&gt; 0.12.13) ([#193](https://github.com/ppat/homelab-ops-actions/issues/193)) ([69cc67e](https://github.com/ppat/homelab-ops-actions/commit/69cc67ef0f81a7f37fec8eb486589edf8267b526))
* **dev-tools:** update astral-sh/uv (0.12.9 -&gt; 0.12.10) ([#190](https://github.com/ppat/homelab-ops-actions/issues/190)) ([38627bb](https://github.com/ppat/homelab-ops-actions/commit/38627bbf3d2aaf719eeb19c794c6e503763048c4))
* **github-actions:** update jdx/mise-action (v4.2.4 -&gt; v4.2.5) ([#194](https://github.com/ppat/homelab-ops-actions/issues/194)) ([9d8b187](https://github.com/ppat/homelab-ops-actions/commit/9d8b187a6fbca422c30a6efc386a3fcf278029b4))

## [2.5.0](https://github.com/ppat/homelab-ops-actions/compare/v2.4.0...v2.5.0) (2026-09-03)


### ✨ Features

* **github-actions:** update ppat/github-workflows (v6.1.0 -&gt; v7.0.0) ([#186](https://github.com/ppat/homelab-ops-actions/issues/186)) ([e922fee](https://github.com/ppat/homelab-ops-actions/commit/e922feeb2697ae7f49f437d6215307d18c6b1fc2))

## [2.4.0](https://github.com/ppat/homelab-ops-actions/compare/v2.3.2...v2.4.0) (2026-09-03)


### ✨ Features

* **github-actions:** update ppat/github-workflows (v6.0.0 -&gt; v6.1.0) ([#182](https://github.com/ppat/homelab-ops-actions/issues/182)) ([a5b0dd5](https://github.com/ppat/homelab-ops-actions/commit/a5b0dd5bc357e95b0fee5fdf218047f4504195d6))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.12.7 -&gt; 0.12.9) ([#181](https://github.com/ppat/homelab-ops-actions/issues/181)) ([45d81d8](https://github.com/ppat/homelab-ops-actions/commit/45d81d8d05fdadc14ec92c3653b1c651b6013631))
* **github-actions:** render diffs with a helm major that matches Flux, and stop failing on digest-pinned charts ([#184](https://github.com/ppat/homelab-ops-actions/issues/184)) ([aaaeda3](https://github.com/ppat/homelab-ops-actions/commit/aaaeda38657c117331e8ad8486d7a73c84f3982a))

## [2.3.2](https://github.com/ppat/homelab-ops-actions/compare/v2.3.1...v2.3.2) (2026-08-31)


### 🚀 Enhancements + Bug Fixes

* skip conflicts created during a sweep ([#179](https://github.com/ppat/homelab-ops-actions/issues/179)) ([cbe4595](https://github.com/ppat/homelab-ops-actions/commit/cbe45952d365ea52d487753f3723a7cf98a86793))

## [2.3.1](https://github.com/ppat/homelab-ops-actions/compare/v2.3.0...v2.3.1) (2026-08-31)


### 🧹 Miscellaneous

* **dev-tools:** update jdx/mise (v2026.8.14 -&gt; v2026.8.15) ([#176](https://github.com/ppat/homelab-ops-actions/issues/176)) ([d0ace26](https://github.com/ppat/homelab-ops-actions/commit/d0ace26444f155be9c6ed193a77ec6622b8402de))


### 🚀 Enhancements + Bug Fixes

* wait for resolved pull-request mergeability ([#178](https://github.com/ppat/homelab-ops-actions/issues/178)) ([31059ed](https://github.com/ppat/homelab-ops-actions/commit/31059edaed5096246adc6a2360a4a32f45318b83))

## [2.3.0](https://github.com/ppat/homelab-ops-actions/compare/v2.2.0...v2.3.0) (2026-08-31)


### 🧹 Miscellaneous

* **dev-tools:** update jdx/mise (v2026.8.10 -&gt; v2026.8.14) ([#174](https://github.com/ppat/homelab-ops-actions/issues/174)) ([f68f6c8](https://github.com/ppat/homelab-ops-actions/commit/f68f6c858c089134e97f99cbff83ee5fadb35225))
* **dev-tools:** update jdx/mise (v2026.8.2 -&gt; v2026.8.5) ([#166](https://github.com/ppat/homelab-ops-actions/issues/166)) ([f6a4d14](https://github.com/ppat/homelab-ops-actions/commit/f6a4d14144b6e5bdbf19e32311c794038e117881))
* **dev-tools:** update jdx/mise (v2026.8.5 -&gt; v2026.8.10) ([#170](https://github.com/ppat/homelab-ops-actions/issues/170)) ([88b2ce4](https://github.com/ppat/homelab-ops-actions/commit/88b2ce43e90b24332cb49f986df3e09e76b6cdab))


### ✨ Features

* add release sweep action ([#175](https://github.com/ppat/homelab-ops-actions/issues/175)) ([3a099d8](https://github.com/ppat/homelab-ops-actions/commit/3a099d86d649401e30342e167f23cf40cade76c4))
* **github-actions:** update ppat/github-workflows (v4.2.0 -&gt; v4.4.0) ([#163](https://github.com/ppat/homelab-ops-actions/issues/163)) ([d8f530f](https://github.com/ppat/homelab-ops-actions/commit/d8f530fae557e1a714c9992d09600a1da0f7387a))
* **github-actions:** update ppat/github-workflows (v4.4.0 -&gt; v5.0.1) ([#168](https://github.com/ppat/homelab-ops-actions/issues/168)) ([e9624a2](https://github.com/ppat/homelab-ops-actions/commit/e9624a27761895fbd81d7fa378af054984075860))
* **github-actions:** update ppat/github-workflows (v5.0.1 -&gt; v6.0.0) ([#169](https://github.com/ppat/homelab-ops-actions/issues/169)) ([ea49fdb](https://github.com/ppat/homelab-ops-actions/commit/ea49fdbb1557496bb51ab4234d8eda0c2169493a))
* update ppat/renovate-presets (v0.2.1 -&gt; v0.3.0) ([#172](https://github.com/ppat/homelab-ops-actions/issues/172)) ([0a408d1](https://github.com/ppat/homelab-ops-actions/commit/0a408d1934c2bb973068a9701896c6ca36d04620))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.12.3 -&gt; 0.12.5) ([#165](https://github.com/ppat/homelab-ops-actions/issues/165)) ([42143b0](https://github.com/ppat/homelab-ops-actions/commit/42143b0bac1a409bdae841bce038ab683420acfc))
* **dev-tools:** update astral-sh/uv (0.12.5 -&gt; 0.12.6) ([#171](https://github.com/ppat/homelab-ops-actions/issues/171)) ([f36f5fe](https://github.com/ppat/homelab-ops-actions/commit/f36f5fec9bc80cea12e0c2e404fb54a2bd73f229))
* **dev-tools:** update astral-sh/uv (0.12.6 -&gt; 0.12.7) ([#173](https://github.com/ppat/homelab-ops-actions/issues/173)) ([f691e7b](https://github.com/ppat/homelab-ops-actions/commit/f691e7b1505522e51b787f74c2ddef647263e68b))

## [2.2.0](https://github.com/ppat/homelab-ops-actions/compare/v2.1.0...v2.2.0) (2026-08-08)


### 🧹 Miscellaneous

* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#152](https://github.com/ppat/homelab-ops-actions/issues/152)) ([d21e276](https://github.com/ppat/homelab-ops-actions/commit/d21e2760d1856872cbe2f04b2a503ff267c52146))
* **dev-tools:** update jdx/mise (v2026.7.15 -&gt; v2026.7.18) ([#156](https://github.com/ppat/homelab-ops-actions/issues/156)) ([4ad7a3d](https://github.com/ppat/homelab-ops-actions/commit/4ad7a3d492e721d9991e812f3dc83be92a22c483))
* **dev-tools:** update jdx/mise (v2026.7.18 -&gt; v2026.8.2) ([#162](https://github.com/ppat/homelab-ops-actions/issues/162)) ([ce7c7d9](https://github.com/ppat/homelab-ops-actions/commit/ce7c7d9c89f79f0c0d3a3ea02bae19cc22327ab8))
* **dev-tools:** update jdx/mise (v2026.7.7 -&gt; v2026.7.15) ([#153](https://github.com/ppat/homelab-ops-actions/issues/153)) ([671d2fe](https://github.com/ppat/homelab-ops-actions/commit/671d2fe5b27d855cbfdf646ebcc033e8ca780c6c))


### ✨ Features

* **dev-tools:** update astral-sh/uv (0.11.33 -&gt; 0.12.0) ([#157](https://github.com/ppat/homelab-ops-actions/issues/157)) ([4be9176](https://github.com/ppat/homelab-ops-actions/commit/4be9176f594abe9ae7a556d706cea7c1d907a2e6))
* **github-actions:** update ppat/github-workflows (v4.1.0 -&gt; v4.2.0) ([#147](https://github.com/ppat/homelab-ops-actions/issues/147)) ([886e96c](https://github.com/ppat/homelab-ops-actions/commit/886e96c40c8aeee79f0dbce701cf9042b9341a44))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.11.29 -&gt; 0.11.31) ([#148](https://github.com/ppat/homelab-ops-actions/issues/148)) ([8561a6c](https://github.com/ppat/homelab-ops-actions/commit/8561a6cbe893c77db8d8ef3be7f489ed26c53480))
* **dev-tools:** update astral-sh/uv (0.11.31 -&gt; 0.11.32) ([#151](https://github.com/ppat/homelab-ops-actions/issues/151)) ([2608e27](https://github.com/ppat/homelab-ops-actions/commit/2608e2758a054ff6bab72561302055dfbe5febf9))
* **dev-tools:** update astral-sh/uv (0.11.32 -&gt; 0.11.33) ([#154](https://github.com/ppat/homelab-ops-actions/issues/154)) ([f07e28a](https://github.com/ppat/homelab-ops-actions/commit/f07e28a0b365293a0553d82d6953b58165fded98))
* **dev-tools:** update astral-sh/uv (0.12.0 -&gt; 0.12.2) ([#158](https://github.com/ppat/homelab-ops-actions/issues/158)) ([9afdc8a](https://github.com/ppat/homelab-ops-actions/commit/9afdc8a78c3055411119235c6a3a5962334fffeb))
* **dev-tools:** update astral-sh/uv (0.12.2 -&gt; 0.12.3) ([#160](https://github.com/ppat/homelab-ops-actions/issues/160)) ([71dca35](https://github.com/ppat/homelab-ops-actions/commit/71dca35fa9978c9eed9a9c644810ea6e65329641))
* **github-actions:** update actions/checkout (v7.0.0 -&gt; v7.0.1) ([#150](https://github.com/ppat/homelab-ops-actions/issues/150)) ([0aab1f1](https://github.com/ppat/homelab-ops-actions/commit/0aab1f15018881e8b5a1168dd41aab889667671d))
* **github-actions:** update jdx/mise-action (v4.2.1 -&gt; v4.2.4) ([#161](https://github.com/ppat/homelab-ops-actions/issues/161)) ([b50ddde](https://github.com/ppat/homelab-ops-actions/commit/b50dddec3392eb75ebd1e6e1dfaaccdee802a5f5))
* give setup-repository-tools extra cache a per-run key with restore-keys ([#159](https://github.com/ppat/homelab-ops-actions/issues/159)) ([5f31b74](https://github.com/ppat/homelab-ops-actions/commit/5f31b74d2299afab4fefbf5d58f8778bcb48ed17)), closes [#155](https://github.com/ppat/homelab-ops-actions/issues/155)

## [2.1.0](https://github.com/ppat/homelab-ops-actions/compare/v2.0.0...v2.1.0) (2026-07-20)


### 🧹 Miscellaneous

* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#143](https://github.com/ppat/homelab-ops-actions/issues/143)) ([f932d9a](https://github.com/ppat/homelab-ops-actions/commit/f932d9a26656c4195df333a71b960034a97c3bcc))
* **dev-tools:** update jdx/mise (v2026.7.0 -&gt; v2026.7.5) ([#140](https://github.com/ppat/homelab-ops-actions/issues/140)) ([859e72a](https://github.com/ppat/homelab-ops-actions/commit/859e72a3244b91cc8a5bb61fff00b1cf9d6071fe))
* **dev-tools:** update jdx/mise (v2026.7.5 -&gt; v2026.7.7) ([#144](https://github.com/ppat/homelab-ops-actions/issues/144)) ([9d4a8e3](https://github.com/ppat/homelab-ops-actions/commit/9d4a8e3cf1e4ff9956059223e9d55c36194de723))


### 🛠 Improvements

* add CLAUDE.md, DESIGN.md, and per-action READMEs ([#142](https://github.com/ppat/homelab-ops-actions/issues/142)) ([680c0da](https://github.com/ppat/homelab-ops-actions/commit/680c0dad4a7b824ad4a4dc27c7767639b330285a))


### ✨ Features

* **github-actions:** update ppat/github-workflows (v3.3.1 -&gt; v4.0.0) ([#136](https://github.com/ppat/homelab-ops-actions/issues/136)) ([df8dff2](https://github.com/ppat/homelab-ops-actions/commit/df8dff2c09eed975cedfc2279c3a65f2b79dc072))
* **github-actions:** update ppat/github-workflows (v4.0.0 -&gt; v4.1.0) ([#146](https://github.com/ppat/homelab-ops-actions/issues/146)) ([396dfac](https://github.com/ppat/homelab-ops-actions/commit/396dfac51fea58f29711dbf26c3413e46d4e898d))


### 🚀 Enhancements + Bug Fixes

* **dev-tools:** update astral-sh/uv (0.11.26 -&gt; 0.11.28) ([#139](https://github.com/ppat/homelab-ops-actions/issues/139)) ([f482c06](https://github.com/ppat/homelab-ops-actions/commit/f482c063d21cc7c8af4b068f19c9dfe73b5f697a))
* **dev-tools:** update astral-sh/uv (0.11.28 -&gt; 0.11.29) ([#141](https://github.com/ppat/homelab-ops-actions/issues/141)) ([32284eb](https://github.com/ppat/homelab-ops-actions/commit/32284eb483cbfc4ca75672941245b1f145efb2e1))
* **github-actions:** update jdx/mise-action (v4.2.0 -&gt; v4.2.1) ([#145](https://github.com/ppat/homelab-ops-actions/issues/145)) ([dba90c1](https://github.com/ppat/homelab-ops-actions/commit/dba90c1577dfb0c360f880265d4179e6c5879af9))
* update ppat/renovate-presets (v0.2.0 -&gt; v0.2.1) ([#135](https://github.com/ppat/homelab-ops-actions/issues/135)) ([9cecb32](https://github.com/ppat/homelab-ops-actions/commit/9cecb32e6b3043d55e7825fb8b5106e4f5c4977c))

## [2.0.0](https://github.com/ppat/homelab-ops-actions/compare/v1.0.4...v2.0.0) (2026-07-03)


### ⚠ BREAKING CHANGES

* **github-actions:** update jdx/mise-action (v3.6.3 -> v4.0.1) ([#95](https://github.com/ppat/homelab-ops-actions/issues/95))
* **github-actions:** update actions/checkout (v4 -> v7.0.0) and actions/cache (v4 -> v6.1.0) ([#130](https://github.com/ppat/homelab-ops-actions/issues/130))

### 🧹 Miscellaneous

* **dev-tools:** update alessandrojcm/commitlint-pre-commit-hook (non-major) ([#129](https://github.com/ppat/homelab-ops-actions/issues/129)) ([a189ec8](https://github.com/ppat/homelab-ops-actions/commit/a189ec87ea550dd8eda331b110b4b98b037a88f1))
* **dev-tools:** update astral-sh/uv (0.11.15 -&gt; 0.11.16) ([#111](https://github.com/ppat/homelab-ops-actions/issues/111)) ([adc7127](https://github.com/ppat/homelab-ops-actions/commit/adc71276db9bea3a5b94e8c174903c34b85e6a41))
* **dev-tools:** update astral-sh/uv (0.11.16 -&gt; 0.11.17) ([#115](https://github.com/ppat/homelab-ops-actions/issues/115)) ([c4e551c](https://github.com/ppat/homelab-ops-actions/commit/c4e551ccb640331931db1d1adbb7dac09aff4467))
* **dev-tools:** update astral-sh/uv (0.11.17 -&gt; 0.11.18) ([#117](https://github.com/ppat/homelab-ops-actions/issues/117)) ([de220a4](https://github.com/ppat/homelab-ops-actions/commit/de220a4cbc9bed44daf0efa33203d30e638d5596))
* **dev-tools:** update astral-sh/uv (0.11.18 -&gt; 0.11.19) ([#118](https://github.com/ppat/homelab-ops-actions/issues/118)) ([ce69a3c](https://github.com/ppat/homelab-ops-actions/commit/ce69a3c979987a32de52c59438ae65c1a1114de3))
* **dev-tools:** update astral-sh/uv (0.11.19 -&gt; 0.11.21) ([#120](https://github.com/ppat/homelab-ops-actions/issues/120)) ([40c2c2b](https://github.com/ppat/homelab-ops-actions/commit/40c2c2b0d76ae0e3f5294b58c263d8849afd7fcf))
* **dev-tools:** update astral-sh/uv (0.11.21 -&gt; 0.11.22) ([#122](https://github.com/ppat/homelab-ops-actions/issues/122)) ([f3907eb](https://github.com/ppat/homelab-ops-actions/commit/f3907eb8000b5983fa01d1b5bd940df9d744efbe))
* **dev-tools:** update astral-sh/uv (0.11.22 -&gt; 0.11.23) ([#123](https://github.com/ppat/homelab-ops-actions/issues/123)) ([8a6c4a2](https://github.com/ppat/homelab-ops-actions/commit/8a6c4a2cc782a404370667bbdb703f03904d1e6c))
* **dev-tools:** update astral-sh/uv (0.11.23 -&gt; 0.11.24) ([#125](https://github.com/ppat/homelab-ops-actions/issues/125)) ([1aa3c6c](https://github.com/ppat/homelab-ops-actions/commit/1aa3c6ceac81868b6cbf533f660c5fba23ff39ee))
* **dev-tools:** update astral-sh/uv (0.11.24 -&gt; 0.11.25) ([#126](https://github.com/ppat/homelab-ops-actions/issues/126)) ([a3ec35d](https://github.com/ppat/homelab-ops-actions/commit/a3ec35d94f9b4ef1b9bc6a448b50ce372de98988))
* **dev-tools:** update astral-sh/uv (0.11.25 -&gt; 0.11.26) ([#128](https://github.com/ppat/homelab-ops-actions/issues/128)) ([fe201e1](https://github.com/ppat/homelab-ops-actions/commit/fe201e1038a0de4c9c5bff883cde09422379e557))
* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#132](https://github.com/ppat/homelab-ops-actions/issues/132)) ([9bd936d](https://github.com/ppat/homelab-ops-actions/commit/9bd936d75d3cf259b2cfaa0a44c2908cd2730983))
* **dev-tools:** update jdx/mise (v2026.5.12 -&gt; v2026.5.16) ([#116](https://github.com/ppat/homelab-ops-actions/issues/116)) ([c3d7257](https://github.com/ppat/homelab-ops-actions/commit/c3d725716bde39359e06e29678c6b03433c88531))
* **dev-tools:** update jdx/mise (v2026.5.16 -&gt; v2026.6.1) ([#119](https://github.com/ppat/homelab-ops-actions/issues/119)) ([b4f6881](https://github.com/ppat/homelab-ops-actions/commit/b4f688180a3d70668001bcd80c716e0e61dcfd92))
* **dev-tools:** update jdx/mise (v2026.5.8 -&gt; v2026.5.12) ([#112](https://github.com/ppat/homelab-ops-actions/issues/112)) ([66634ec](https://github.com/ppat/homelab-ops-actions/commit/66634ec6def68fe932d0b1db7cefa4c3fd37d2ca))
* **dev-tools:** update jdx/mise (v2026.6.1 -&gt; v2026.6.10) ([#121](https://github.com/ppat/homelab-ops-actions/issues/121)) ([4e7d81d](https://github.com/ppat/homelab-ops-actions/commit/4e7d81d38bdb45de97ec69e62ec353262436a1bc))
* **dev-tools:** update jdx/mise (v2026.6.10 -&gt; v2026.6.11) ([#124](https://github.com/ppat/homelab-ops-actions/issues/124)) ([c059316](https://github.com/ppat/homelab-ops-actions/commit/c059316935794fd079e910506d51106bccfa4884))
* **dev-tools:** update jdx/mise (v2026.6.11 -&gt; v2026.6.14) ([#127](https://github.com/ppat/homelab-ops-actions/issues/127)) ([3e250e4](https://github.com/ppat/homelab-ops-actions/commit/3e250e4b263befed71cfd48166df0b9efa675acc))
* **dev-tools:** update jdx/mise (v2026.6.14 -&gt; v2026.7.0) ([#131](https://github.com/ppat/homelab-ops-actions/issues/131)) ([4300077](https://github.com/ppat/homelab-ops-actions/commit/4300077c33417074e60bef3e4a1669f99f4fbbca))
* **github-actions:** update ppat/github-workflows (v3.2.0 -&gt; v3.3.0) ([#110](https://github.com/ppat/homelab-ops-actions/issues/110)) ([ab67157](https://github.com/ppat/homelab-ops-actions/commit/ab6715733a680ee7c057999c7c354067c6abcb0c))
* **github-actions:** update ppat/github-workflows (v3.3.0 -&gt; v3.3.1) ([#114](https://github.com/ppat/homelab-ops-actions/issues/114)) ([61e7bf6](https://github.com/ppat/homelab-ops-actions/commit/61e7bf62b5c97b3e1f454a7381bf632ae7e7905a))


### ✨ Features

* **github-actions:** update actions/checkout (v4 -&gt; v7.0.0) and actions/cache (v4 -&gt; v6.1.0) ([#130](https://github.com/ppat/homelab-ops-actions/issues/130)) ([69b72e8](https://github.com/ppat/homelab-ops-actions/commit/69b72e89ac35121d73cdc8e53214f16a26473936))
* **github-actions:** update jdx/mise-action (v3.6.3 -&gt; v4.0.1) ([#95](https://github.com/ppat/homelab-ops-actions/issues/95)) ([b9d1cee](https://github.com/ppat/homelab-ops-actions/commit/b9d1cee4a18ef085ccebf16a564c915a40d2b9c7))
* **github-actions:** update jdx/mise-action (v4.0.1 -&gt; v4.2.0) ([#133](https://github.com/ppat/homelab-ops-actions/issues/133)) ([f892490](https://github.com/ppat/homelab-ops-actions/commit/f8924906199418ddee43e8f4fdf3446555f33f05))
* update ppat/renovate-presets (v0.1.1 -&gt; v0.2.0) ([#134](https://github.com/ppat/homelab-ops-actions/issues/134)) ([3f4c9ce](https://github.com/ppat/homelab-ops-actions/commit/3f4c9ce90d0da0ec923152947e518556651675d0))

## [1.0.4](https://github.com/ppat/homelab-ops-actions/compare/v1.0.3...v1.0.4) (2026-05-21)


### 🧹 Miscellaneous

* **dev-tools:** update alessandrojcm/commitlint-pre-commit-hook (non-major) ([#108](https://github.com/ppat/homelab-ops-actions/issues/108)) ([d89fcb4](https://github.com/ppat/homelab-ops-actions/commit/d89fcb44dc1feaa827c9e6c686f7097b11d93d17))
* **dev-tools:** update astral-sh/uv (0.10.0 -&gt; 0.10.2) ([#70](https://github.com/ppat/homelab-ops-actions/issues/70)) ([0a5b49e](https://github.com/ppat/homelab-ops-actions/commit/0a5b49e2f6790c745e1e3c3da9f4b5f9b89430c4))
* **dev-tools:** update astral-sh/uv (0.10.10 -&gt; 0.10.11) ([#85](https://github.com/ppat/homelab-ops-actions/issues/85)) ([60fcf9e](https://github.com/ppat/homelab-ops-actions/commit/60fcf9e18cb36d3eb122402cb0fea11f5f590e53))
* **dev-tools:** update astral-sh/uv (0.10.11 -&gt; 0.10.12) ([#86](https://github.com/ppat/homelab-ops-actions/issues/86)) ([3c93bbe](https://github.com/ppat/homelab-ops-actions/commit/3c93bbe48526bbd7b276958cd61b5b25d3d27120))
* **dev-tools:** update astral-sh/uv (0.10.12 -&gt; 0.11.1) ([#91](https://github.com/ppat/homelab-ops-actions/issues/91)) ([bb2f348](https://github.com/ppat/homelab-ops-actions/commit/bb2f348bb4a2cd639f0698736cbee83ad6eacaf2))
* **dev-tools:** update astral-sh/uv (0.10.2 -&gt; 0.10.4) ([#72](https://github.com/ppat/homelab-ops-actions/issues/72)) ([b324bad](https://github.com/ppat/homelab-ops-actions/commit/b324badca6700191b25ef183185595f3ea23f17b))
* **dev-tools:** update astral-sh/uv (0.10.4 -&gt; 0.10.6) ([#74](https://github.com/ppat/homelab-ops-actions/issues/74)) ([1bebd09](https://github.com/ppat/homelab-ops-actions/commit/1bebd0992acffcb5d72a1884cc7bdc07f970e108))
* **dev-tools:** update astral-sh/uv (0.10.6 -&gt; 0.10.7) ([#75](https://github.com/ppat/homelab-ops-actions/issues/75)) ([c556d48](https://github.com/ppat/homelab-ops-actions/commit/c556d48a41deabaa781efa913a03804decad4189))
* **dev-tools:** update astral-sh/uv (0.10.7 -&gt; 0.10.8) ([#79](https://github.com/ppat/homelab-ops-actions/issues/79)) ([142d481](https://github.com/ppat/homelab-ops-actions/commit/142d4810ca764d17d0e3c1e33a3c8b069c1713d3))
* **dev-tools:** update astral-sh/uv (0.10.8 -&gt; 0.10.9) ([#80](https://github.com/ppat/homelab-ops-actions/issues/80)) ([182ac99](https://github.com/ppat/homelab-ops-actions/commit/182ac99f4f20a0ee9a904c0eb1c8bf404acf32b2))
* **dev-tools:** update astral-sh/uv (0.10.9 -&gt; 0.10.10) ([#82](https://github.com/ppat/homelab-ops-actions/issues/82)) ([ee4aa1e](https://github.com/ppat/homelab-ops-actions/commit/ee4aa1ee0dbe47fdf8b06c4a0ffd999220461fe7))
* **dev-tools:** update astral-sh/uv (0.11.1 -&gt; 0.11.3) ([#92](https://github.com/ppat/homelab-ops-actions/issues/92)) ([1dd61c6](https://github.com/ppat/homelab-ops-actions/commit/1dd61c62daeb51e8a1dab252b3c578778a3d09ff))
* **dev-tools:** update astral-sh/uv (0.11.10 -&gt; 0.11.11) ([#103](https://github.com/ppat/homelab-ops-actions/issues/103)) ([a8ed267](https://github.com/ppat/homelab-ops-actions/commit/a8ed267de147d27c0811acbcb37b0ddef17e602b))
* **dev-tools:** update astral-sh/uv (0.11.11 -&gt; 0.11.12) ([#105](https://github.com/ppat/homelab-ops-actions/issues/105)) ([07aa2d1](https://github.com/ppat/homelab-ops-actions/commit/07aa2d113c43e0e560c17ef710afa4ad641c8335))
* **dev-tools:** update astral-sh/uv (0.11.12 -&gt; 0.11.14) ([#106](https://github.com/ppat/homelab-ops-actions/issues/106)) ([2be4a47](https://github.com/ppat/homelab-ops-actions/commit/2be4a4774cfe437696d0389b465c96d5312bc65b))
* **dev-tools:** update astral-sh/uv (0.11.14 -&gt; 0.11.15) ([#109](https://github.com/ppat/homelab-ops-actions/issues/109)) ([25cc89b](https://github.com/ppat/homelab-ops-actions/commit/25cc89b64759d168b8f2e80ffa0f7ee4031c5685))
* **dev-tools:** update astral-sh/uv (0.11.3 -&gt; 0.11.6) ([#94](https://github.com/ppat/homelab-ops-actions/issues/94)) ([0644b26](https://github.com/ppat/homelab-ops-actions/commit/0644b26c7a5ba0569f74db39085a5d01bfc3f1a3))
* **dev-tools:** update astral-sh/uv (0.11.6 -&gt; 0.11.7) ([#96](https://github.com/ppat/homelab-ops-actions/issues/96)) ([0793717](https://github.com/ppat/homelab-ops-actions/commit/0793717e66799134860f303a8ab0420b28a67bc7))
* **dev-tools:** update astral-sh/uv (0.11.7 -&gt; 0.11.8) ([#98](https://github.com/ppat/homelab-ops-actions/issues/98)) ([a4a337e](https://github.com/ppat/homelab-ops-actions/commit/a4a337e6bf9aa248198a4828d27ad23144d0c152))
* **dev-tools:** update astral-sh/uv (0.11.8 -&gt; 0.11.10) ([#102](https://github.com/ppat/homelab-ops-actions/issues/102)) ([be26179](https://github.com/ppat/homelab-ops-actions/commit/be2617945afed9c37437e9b85eed80f6567bc4d1))
* **dev-tools:** update astral-sh/uv (0.9.17 -&gt; 0.9.18) ([#50](https://github.com/ppat/homelab-ops-actions/issues/50)) ([4488be7](https://github.com/ppat/homelab-ops-actions/commit/4488be7b9c2e59bd6dc4f09d31412ae65d9cec1f))
* **dev-tools:** update astral-sh/uv (0.9.18 -&gt; 0.9.20) ([#52](https://github.com/ppat/homelab-ops-actions/issues/52)) ([70a6a88](https://github.com/ppat/homelab-ops-actions/commit/70a6a8891136a233bbe29afdcb74f570d5305a27))
* **dev-tools:** update astral-sh/uv (0.9.20 -&gt; 0.9.21) ([#53](https://github.com/ppat/homelab-ops-actions/issues/53)) ([7e4119e](https://github.com/ppat/homelab-ops-actions/commit/7e4119eaa2d0e4d8f344b6e74a88246259f4ad4c))
* **dev-tools:** update astral-sh/uv (0.9.21 -&gt; 0.9.22) ([#55](https://github.com/ppat/homelab-ops-actions/issues/55)) ([bb6046a](https://github.com/ppat/homelab-ops-actions/commit/bb6046ae0f5924108214318f3aa33bbf5f3bcb33))
* **dev-tools:** update astral-sh/uv (0.9.22 -&gt; 0.9.24) ([#57](https://github.com/ppat/homelab-ops-actions/issues/57)) ([54d0d32](https://github.com/ppat/homelab-ops-actions/commit/54d0d32aacc6f75b8727bab752b4b7e99c29e9ea))
* **dev-tools:** update astral-sh/uv (0.9.24 -&gt; 0.9.25) ([#58](https://github.com/ppat/homelab-ops-actions/issues/58)) ([fd43e69](https://github.com/ppat/homelab-ops-actions/commit/fd43e69529bdf8520795a03157adf0949691411c))
* **dev-tools:** update astral-sh/uv (0.9.25 -&gt; 0.9.26) ([#61](https://github.com/ppat/homelab-ops-actions/issues/61)) ([9c0b73b](https://github.com/ppat/homelab-ops-actions/commit/9c0b73b2a9a165bc626fa23485e33a79579cb052))
* **dev-tools:** update astral-sh/uv (0.9.26 -&gt; 0.9.27) ([#63](https://github.com/ppat/homelab-ops-actions/issues/63)) ([d5c62cf](https://github.com/ppat/homelab-ops-actions/commit/d5c62cf96bd8ca35b04f5a5d70bec71042a8a465))
* **dev-tools:** update astral-sh/uv (0.9.27 -&gt; 0.9.28) ([#66](https://github.com/ppat/homelab-ops-actions/issues/66)) ([99892ef](https://github.com/ppat/homelab-ops-actions/commit/99892ef987526fd3eef887142ed2c74b500b61da))
* **dev-tools:** update astral-sh/uv (0.9.28 -&gt; 0.9.30) ([#68](https://github.com/ppat/homelab-ops-actions/issues/68)) ([a1a1e55](https://github.com/ppat/homelab-ops-actions/commit/a1a1e5578390dd359d36376eb447af5e1cc737d3))
* **dev-tools:** update astral-sh/uv (0.9.30 -&gt; 0.10.0) ([#69](https://github.com/ppat/homelab-ops-actions/issues/69)) ([06b2315](https://github.com/ppat/homelab-ops-actions/commit/06b23158f33970fd1c9924fcf486306f3cad9c0d))
* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#77](https://github.com/ppat/homelab-ops-actions/issues/77)) ([9b36b94](https://github.com/ppat/homelab-ops-actions/commit/9b36b949e814ee6778877d702b3f0c716de28bd5))
* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#90](https://github.com/ppat/homelab-ops-actions/issues/90)) ([7eb525b](https://github.com/ppat/homelab-ops-actions/commit/7eb525b1783a6fad72fb14f9e8a5a20f4004e211))
* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#99](https://github.com/ppat/homelab-ops-actions/issues/99)) ([5eb983e](https://github.com/ppat/homelab-ops-actions/commit/5eb983e815f337e412fc4e1fa1db4b027d756a9c))
* **dev-tools:** update jdx/mise (v2025.12.12 -&gt; v2025.12.13) ([#56](https://github.com/ppat/homelab-ops-actions/issues/56)) ([f1f95e9](https://github.com/ppat/homelab-ops-actions/commit/f1f95e9d7a8197c86b371603187d6b5348cdd178))
* **dev-tools:** update jdx/mise (v2025.12.13 -&gt; v2026.1.3) ([#59](https://github.com/ppat/homelab-ops-actions/issues/59)) ([cf4e731](https://github.com/ppat/homelab-ops-actions/commit/cf4e7314c2d5273203e687eab9ec37edd4614bd5))
* **dev-tools:** update jdx/mise (v2025.12.2 -&gt; v2025.12.12) ([#51](https://github.com/ppat/homelab-ops-actions/issues/51)) ([985f7e4](https://github.com/ppat/homelab-ops-actions/commit/985f7e4d9906e839f2ed3e8a284d1377e7ae6312))
* **dev-tools:** update jdx/mise (v2026.1.3 -&gt; v2026.1.5) ([#62](https://github.com/ppat/homelab-ops-actions/issues/62)) ([1080a90](https://github.com/ppat/homelab-ops-actions/commit/1080a90f31add8e1d5980015834f826ab5a8aa88))
* **dev-tools:** update jdx/mise (v2026.1.5 -&gt; v2026.1.8) ([#64](https://github.com/ppat/homelab-ops-actions/issues/64)) ([9e3898d](https://github.com/ppat/homelab-ops-actions/commit/9e3898d6cb062a62c62abd5213fbbb22836c0172))
* **dev-tools:** update jdx/mise (v2026.1.8 -&gt; v2026.2.11) ([#67](https://github.com/ppat/homelab-ops-actions/issues/67)) ([ea6380e](https://github.com/ppat/homelab-ops-actions/commit/ea6380eaf624599ecf4e25de92c30b72fd473d14))
* **dev-tools:** update jdx/mise (v2026.2.11 -&gt; v2026.2.17) ([#73](https://github.com/ppat/homelab-ops-actions/issues/73)) ([44057df](https://github.com/ppat/homelab-ops-actions/commit/44057df3502385b3a0b3b39d67ee9c13480dc3df))
* **dev-tools:** update jdx/mise (v2026.2.17 -&gt; v2026.3.3) ([#76](https://github.com/ppat/homelab-ops-actions/issues/76)) ([0058d7d](https://github.com/ppat/homelab-ops-actions/commit/0058d7d9f67851efe08baa2cf66ca9cb74a98612))
* **dev-tools:** update jdx/mise (v2026.3.17 -&gt; v2026.4.11) ([#93](https://github.com/ppat/homelab-ops-actions/issues/93)) ([a2c2250](https://github.com/ppat/homelab-ops-actions/commit/a2c2250bfa1d5983b9bc28a4d6f80392694bab26))
* **dev-tools:** update jdx/mise (v2026.3.3 -&gt; v2026.3.5) ([#81](https://github.com/ppat/homelab-ops-actions/issues/81)) ([a178f73](https://github.com/ppat/homelab-ops-actions/commit/a178f734a74f969528ea4c8b5b0ada6c600966e0))
* **dev-tools:** update jdx/mise (v2026.3.5 -&gt; v2026.3.8) ([#83](https://github.com/ppat/homelab-ops-actions/issues/83)) ([fbbed22](https://github.com/ppat/homelab-ops-actions/commit/fbbed22d1b32428d87ad68bb5dd1ffe3b1cc3495))
* **dev-tools:** update jdx/mise (v2026.3.8 -&gt; v2026.3.9) ([#88](https://github.com/ppat/homelab-ops-actions/issues/88)) ([40c9dad](https://github.com/ppat/homelab-ops-actions/commit/40c9dad20d66fe779761a2a15d2a48867159e16f))
* **dev-tools:** update jdx/mise (v2026.3.9 -&gt; v2026.3.17) ([#89](https://github.com/ppat/homelab-ops-actions/issues/89)) ([5d14747](https://github.com/ppat/homelab-ops-actions/commit/5d147478233072e2c310274f487324c30ec6f12e))
* **dev-tools:** update jdx/mise (v2026.4.11 -&gt; v2026.4.18) ([#97](https://github.com/ppat/homelab-ops-actions/issues/97)) ([5b7529f](https://github.com/ppat/homelab-ops-actions/commit/5b7529fa1a34e54b8c4156e5e4da4ad8047b04e8))
* **dev-tools:** update jdx/mise (v2026.4.18 -&gt; v2026.4.23) ([#100](https://github.com/ppat/homelab-ops-actions/issues/100)) ([2ed7449](https://github.com/ppat/homelab-ops-actions/commit/2ed7449eaf7f9bd757ab13e1407b4edc6aac0a5b))
* **dev-tools:** update jdx/mise (v2026.4.23 -&gt; v2026.4.28) ([#101](https://github.com/ppat/homelab-ops-actions/issues/101)) ([2e4ec44](https://github.com/ppat/homelab-ops-actions/commit/2e4ec444ed10b404ea34a18aa9a5864ca1c3429a))
* **dev-tools:** update jdx/mise (v2026.4.28 -&gt; v2026.5.1) ([#104](https://github.com/ppat/homelab-ops-actions/issues/104)) ([bfd4e3c](https://github.com/ppat/homelab-ops-actions/commit/bfd4e3c2ad0e5c6b2180adfa8a34d597712f9e33))
* **dev-tools:** update jdx/mise (v2026.5.1 -&gt; v2026.5.8) ([#107](https://github.com/ppat/homelab-ops-actions/issues/107)) ([e2778d1](https://github.com/ppat/homelab-ops-actions/commit/e2778d17b936d2988ab33a17705a841dc804c4a9))
* **dev-tools:** update pre-commit packages (non-major) ([#65](https://github.com/ppat/homelab-ops-actions/issues/65)) ([173d9e4](https://github.com/ppat/homelab-ops-actions/commit/173d9e449953fca6ce6c0fc178db20a7c2a5ed14))
* **github-actions:** update jdx/mise-action (v3.4.1 -&gt; v3.5.1) ([#47](https://github.com/ppat/homelab-ops-actions/issues/47)) ([90b0bd6](https://github.com/ppat/homelab-ops-actions/commit/90b0bd68233e2f360a0cd6d70d9748d7d6a24df2))
* **github-actions:** update jdx/mise-action (v3.5.1 -&gt; v3.6.1) ([#78](https://github.com/ppat/homelab-ops-actions/issues/78)) ([f7aec93](https://github.com/ppat/homelab-ops-actions/commit/f7aec9357ee0a11ee49feb37fbc8490147a7b46f))
* **github-actions:** update jdx/mise-action (v3.6.1 -&gt; v3.6.3) ([#84](https://github.com/ppat/homelab-ops-actions/issues/84)) ([b2bc321](https://github.com/ppat/homelab-ops-actions/commit/b2bc321257d8f3e877680f56b24f84d622e5cb6b))
* **github-actions:** update ppat/github-workflows (v3.1.0 -&gt; v3.2.0) ([#49](https://github.com/ppat/homelab-ops-actions/issues/49)) ([9653685](https://github.com/ppat/homelab-ops-actions/commit/96536852f0d866df239cf1f1cfe0e651e0cbbe8d))


### 🚀 Enhancements + Bug Fixes

* update ppat/renovate-presets (v0.1.0 -&gt; v0.1.1) ([#87](https://github.com/ppat/homelab-ops-actions/issues/87)) ([11bd057](https://github.com/ppat/homelab-ops-actions/commit/11bd057f099a24ea0f6dfdc9a1eeeb3c1c416241))

## [1.0.3](https://github.com/ppat/homelab-ops-actions/compare/v1.0.2...v1.0.3) (2025-12-16)


### 🧹 Miscellaneous

* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#41](https://github.com/ppat/homelab-ops-actions/issues/41)) ([604828f](https://github.com/ppat/homelab-ops-actions/commit/604828fa43e10ed17ae74392f58b56b3b9deaa82))
* **dev-tools:** update jdx/mise (v2025.11.4 -&gt; v2025.12.2) ([#46](https://github.com/ppat/homelab-ops-actions/issues/46)) ([d52791d](https://github.com/ppat/homelab-ops-actions/commit/d52791d12c3ce5d419f7de0334e612320e9bf98e))
* **github-actions:** update ghcr.io/allenporter/flux-local (v7.10.1 -&gt; v8.0.1) ([#33](https://github.com/ppat/homelab-ops-actions/issues/33)) ([d718aa1](https://github.com/ppat/homelab-ops-actions/commit/d718aa1199e2d0d0a8e38e7f2b013e389e3dce0d))
* **github-actions:** update jdx/mise-action (v3.4.0 -&gt; v3.4.1) ([#40](https://github.com/ppat/homelab-ops-actions/issues/40)) ([926576f](https://github.com/ppat/homelab-ops-actions/commit/926576f7e2b50e721aa0ff3e55af0f3fe3805392))
* **github-actions:** update ppat/github-workflows (v3.0.1 -&gt; v3.1.0) ([#37](https://github.com/ppat/homelab-ops-actions/issues/37)) ([dffabac](https://github.com/ppat/homelab-ops-actions/commit/dffabac6f3e3f31e2c58c6bb0762fd845cc7e203))


### 🛠 Improvements

* add test for setup-repository-tools action ([#43](https://github.com/ppat/homelab-ops-actions/issues/43)) ([15cb9da](https://github.com/ppat/homelab-ops-actions/commit/15cb9da779346a07b925731e4b2e1b0a95207cac))
* add tests for flux-diff + comment-on-pr github actions ([#39](https://github.com/ppat/homelab-ops-actions/issues/39)) ([e1cd2ac](https://github.com/ppat/homelab-ops-actions/commit/e1cd2ac76a1fc2ada94333527ef11f8b8c27af2a))
* fix flux-diff test issue when run on workflow dispatch or schedule ([#45](https://github.com/ppat/homelab-ops-actions/issues/45)) ([c708fe3](https://github.com/ppat/homelab-ops-actions/commit/c708fe38a426d71d4de82f614acef2310d425037))
* minor changes to flux-diff test ([#44](https://github.com/ppat/homelab-ops-actions/issues/44)) ([bb27699](https://github.com/ppat/homelab-ops-actions/commit/bb2769934d7efeaaaac06320d9b5cb4d3d93382b))

## [1.0.2](https://github.com/ppat/homelab-ops-actions/compare/v1.0.1...v1.0.2) (2025-12-13)


### 🧹 Miscellaneous

* **dev-tools:** update davidanson/markdownlint-cli2 (non-major) ([#35](https://github.com/ppat/homelab-ops-actions/issues/35)) ([791dbb5](https://github.com/ppat/homelab-ops-actions/commit/791dbb5a42f311a0962f351c4a437be6000c71f1))
* **github-actions:** update digest actions/checkout (08eba0b -&gt; 34e1148) ([#34](https://github.com/ppat/homelab-ops-actions/issues/34)) ([d00df6a](https://github.com/ppat/homelab-ops-actions/commit/d00df6acd0e3d7575cbd9e5afc371df1a3e63044))
* **github-actions:** update jdx/mise-action (v3.3.1 -&gt; v3.4.0) ([#36](https://github.com/ppat/homelab-ops-actions/issues/36)) ([aa5b62a](https://github.com/ppat/homelab-ops-actions/commit/aa5b62af177960ae903ab2980d82ff66c8b47afd))
* **github-actions:** update ppat/github-workflows (v3.0.0 -&gt; v3.0.1) ([#31](https://github.com/ppat/homelab-ops-actions/issues/31)) ([19826a3](https://github.com/ppat/homelab-ops-actions/commit/19826a34c303c5b9b52f2d6c9a1beab46df79448))

## [1.0.1](https://github.com/ppat/homelab-ops-actions/compare/v1.0.0...v1.0.1) (2025-10-21)


### 🧹 Miscellaneous

* **github-actions:** update ppat/github-workflows (v2.1.1 -&gt; v3.0.0) ([#28](https://github.com/ppat/homelab-ops-actions/issues/28)) ([c44866e](https://github.com/ppat/homelab-ops-actions/commit/c44866e26b208205552469533ec7eeb97dc0ba30))

## [1.0.0](https://github.com/ppat/homelab-ops-actions/compare/v0.0.3...v1.0.0) (2025-10-18)


### ⚠ BREAKING CHANGES

* parameterize tools mise.toml as action input param instead of passing in a tools repo ([#26](https://github.com/ppat/homelab-ops-actions/issues/26))

### 🧹 Miscellaneous

* **dev-tools:** update alessandrojcm/commitlint-pre-commit-hook (non-major) ([#23](https://github.com/ppat/homelab-ops-actions/issues/23)) ([1989811](https://github.com/ppat/homelab-ops-actions/commit/19898119743c869d680bcb38446f0faa0b1740a6))
* **dev-tools:** update pre-commit/pre-commit-hooks (major) ([#17](https://github.com/ppat/homelab-ops-actions/issues/17)) ([dda5433](https://github.com/ppat/homelab-ops-actions/commit/dda54332e942f9a2342d00b79b185aa42c32db96))
* **github-actions:** update digest actions/cache (0400d5f -&gt; 0057852) ([#21](https://github.com/ppat/homelab-ops-actions/issues/21)) ([93f2aa5](https://github.com/ppat/homelab-ops-actions/commit/93f2aa52e7def79d23dfda12375eb6258fb8fefa))
* **github-actions:** update ghcr.io/allenporter/flux-local (v7.10.0 -&gt; v7.10.1) ([#22](https://github.com/ppat/homelab-ops-actions/issues/22)) ([01bcdc4](https://github.com/ppat/homelab-ops-actions/commit/01bcdc42886bbaff89527dd70123ba9fb1896fcf))
* **github-actions:** update ghcr.io/allenporter/flux-local (v7.8.0 -&gt; v7.10.0) ([#16](https://github.com/ppat/homelab-ops-actions/issues/16)) ([603a943](https://github.com/ppat/homelab-ops-actions/commit/603a943b8ebd2755e6fc81e02673f9b666b8074f))
* **github-actions:** update jdx/mise-action (v2.4.4 -&gt; v3.2.0) ([#19](https://github.com/ppat/homelab-ops-actions/issues/19)) ([409f884](https://github.com/ppat/homelab-ops-actions/commit/409f88437e5646f351f91a5f94050f532b38aff8))
* **github-actions:** update jdx/mise-action (v3.2.0 -&gt; v3.3.1) ([#24](https://github.com/ppat/homelab-ops-actions/issues/24)) ([a06bc28](https://github.com/ppat/homelab-ops-actions/commit/a06bc2896f23ea9fd99e2f58a6298f0ad5a3ccc6))
* **github-actions:** update ppat/github-workflows (v2.0.1 -&gt; v2.0.3) ([#13](https://github.com/ppat/homelab-ops-actions/issues/13)) ([7e9410d](https://github.com/ppat/homelab-ops-actions/commit/7e9410dddc2c8264d7b58e50afddd97bea896845))
* **github-actions:** update ppat/github-workflows (v2.0.3 -&gt; v2.1.1) ([#20](https://github.com/ppat/homelab-ops-actions/issues/20)) ([878fc6d](https://github.com/ppat/homelab-ops-actions/commit/878fc6d5825708e4373f8e8112512542fbf3b52a))


### ✨ Features

* update ppat/renovate-presets (v0.0.4 -&gt; v0.1.0) ([#27](https://github.com/ppat/homelab-ops-actions/issues/27)) ([bb494e5](https://github.com/ppat/homelab-ops-actions/commit/bb494e51c805a57a81e94c9d8d0ad4617f323c5d))


### 🚀 Enhancements + Bug Fixes

* parameterize tools mise.toml as action input param instead of passing in a tools repo ([#26](https://github.com/ppat/homelab-ops-actions/issues/26)) ([1c743a3](https://github.com/ppat/homelab-ops-actions/commit/1c743a3d6ec0037aa73d2625217c46e67ade9682))
* update ppat/renovate-presets (v0.0.3 -&gt; v0.0.4) ([#14](https://github.com/ppat/homelab-ops-actions/issues/14)) ([7b9e089](https://github.com/ppat/homelab-ops-actions/commit/7b9e08997bec7e583fb5098132751747c0ff5f4f))

## [0.0.3](https://github.com/ppat/homelab-ops-actions/compare/v0.0.2...v0.0.3) (2025-08-24)


### 🧹 Miscellaneous

* **github-actions:** update digest trusted github-actions ([#9](https://github.com/ppat/homelab-ops-actions/issues/9)) ([d68761f](https://github.com/ppat/homelab-ops-actions/commit/d68761ff7dfa8b1856636d28a024c3dcda109cdb))
* **github-actions:** update ghcr.io/allenporter/flux-local (v7.5.4 -&gt; v7.8.0) ([#10](https://github.com/ppat/homelab-ops-actions/issues/10)) ([e0d735c](https://github.com/ppat/homelab-ops-actions/commit/e0d735c78eabb9db5c7443bb6b91940a8da9adf6))
* **github-actions:** update ppat/github-workflows (v2.0.0 -&gt; v2.0.1) ([#8](https://github.com/ppat/homelab-ops-actions/issues/8)) ([642746b](https://github.com/ppat/homelab-ops-actions/commit/642746b6b96b7e1b1817b5e8465677cc29b0850d))


### 🚀 Enhancements + Bug Fixes

* start mise cache anew monthly in setup-repository-tools action ([#12](https://github.com/ppat/homelab-ops-actions/issues/12)) ([332ac07](https://github.com/ppat/homelab-ops-actions/commit/332ac078c568f636f3e7a772ada67391b6fdd328))

## [0.0.2](https://github.com/ppat/homelab-ops-actions/compare/v0.0.1...v0.0.2) (2025-08-07)


### 🧹 Miscellaneous

* **github-actions:** update ppat/github-workflows (v1.1.1 -&gt; v2.0.0) ([#5](https://github.com/ppat/homelab-ops-actions/issues/5)) ([8f7d21a](https://github.com/ppat/homelab-ops-actions/commit/8f7d21a8ccb9b7605e5c0a07dfe4aadbe45bf589))


### ✨ Features

* add actions - flux-diff and comment-on-pr ([#7](https://github.com/ppat/homelab-ops-actions/issues/7)) ([2b70fe7](https://github.com/ppat/homelab-ops-actions/commit/2b70fe71d560f4e1d7f882a75fb3f05b09e826e9))

## 0.0.1 (2025-08-03)


### 🧹 Miscellaneous

* initial commit ([87d162f](https://github.com/ppat/homelab-ops-actions/commit/87d162f6363291cae66d0fc262bc87aca8f0ffa5))


### ✨ Features

* add create-signed-commit action ([#3](https://github.com/ppat/homelab-ops-actions/issues/3)) ([6bc61ee](https://github.com/ppat/homelab-ops-actions/commit/6bc61ee7a3540cbb99bee7d6332814cc81781ccd))
* add setup-repository-tools action ([#1](https://github.com/ppat/homelab-ops-actions/issues/1)) ([5afb6e9](https://github.com/ppat/homelab-ops-actions/commit/5afb6e98e4c3a5a71e626ac41ee4412e27bb9139))
